...
import { PINNED_TOOLCHAIN } from "../version.js";
...
const resolveNativeTsgo = (
  ...
  const platformPackagePath = require.resolve(
...
const resolvePaths = (): ToolchainPaths => {
  ...
  const sourceDoctorPlugin = fileURLToPath(
    new URL("doctor-plugin.ts", import.meta.url)
  );
  const builtDoctorPlugin = fileURLToPath(
    new URL("doctor-plugin.js", import.meta.url)
  );

  return {
    doctorPlugin: import.meta.url.endsWith(".ts")
      ? sourceDoctorPlugin
      : builtDoctorPlugin,
    effectPlugin,
